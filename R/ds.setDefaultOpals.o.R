#' @title ds.setDefaultOpals creates a default set of Opal objects called 'default.connections'
...
#' written so that if one of the Opal sets is called 'default.connections' then that set -
#' i.e. 'default.connections' - will be selected by default by all DataSHIELD client-side functions. If there is more
#' than one set of Opals in the analytic environment and NONE of
#' these is called 'default.connections', the function ds.setDefaultOpals() therefore copies
#' one set of opals and to name that copy 'default.connections'. This set will then be selected
#' by default by all client-side functions, unless it is deleted and an alternative set of
#' opals is copied and named 'default.connections'. Regardless how many sets of opals exist and
#' regardless whether any of them may be called 'default.connections', the 'datasources=' argument
...
#' and calls the copy 'default.connections'
